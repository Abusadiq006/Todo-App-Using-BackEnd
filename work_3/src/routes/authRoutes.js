...
router.post('/register', (req, res) => {
    ...
    // Save abusadiq@gmail.com | askdshhfcx.sdef.dfdsd.f...sdsdws
...
router.post('/login', (req, res) => {
    // we get their email, and look up the password associated with tha email in the database
    ...
    try {
        ...
        if (!passwordIsValid) {return res.status(401).send({message: 
            "Invalid password"}) }
            console.log(user)
            // if the password is correct then we have a successful authentication
